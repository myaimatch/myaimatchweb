...
class AnimationController {
  ...
  public render() {
    const ctx = this.ctx
    if (!ctx) return
    // Transparent fill so our hero background shows through
    ctx.clearRect(0, 0, this.size * this.dpr, this.size * this.dpr)
...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      // Access private fields via bracket notation (needed for projection)
      const cameraZ = (controller as unknown as { cameraZ: number }).cameraZ
      const viewZoom = (controller as unknown as { viewZoom: number }).viewZoom
...
export function SpiralAnimation() {
  ...
  // SSR-safe default — useEffect updates with real window size
