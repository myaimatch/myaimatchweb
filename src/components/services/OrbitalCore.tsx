  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      [0.28, 0.39, 0.5].forEach((radiusFactor, i) => {
        ...
        ctx.strokeStyle = `rgba(223,122,254,${0.46 - i * 0.06})`;
        ctx.lineWidth = 1.6;
        ctx.stroke();
        ...
        for (let n = 0; n < count; n++) {
          ...
          ctx.fillStyle = n % 2 ? `rgba(255,255,255,${0.48 + depth * 0.34})` : `rgba(223,122,254,${0.58 + depth * 0.38})`;
          ctx.fill();
        }
      });
      ...
      halo.addColorStop(0, "rgba(223,122,254,0.72)");
      ...
      ctx.lineWidth = 1.4;
      ctx.beginPath();
      ctx.arc(cx, cy, min * 0.085 * corePulse, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
